...
/// _amountsList is an arrpexof:pex
...
//orderInfoList[q++] = bytes32(state.pexlAmountS);
//orderInfoList[q++] = bytes32(state.pexReward);
//orderIpexList[q++] = bytes32(pexpex
...
func (e *RingMinedEvent) ConvertDown() (*types.RingMinedEvent, []*types.OrderFilledEvent, error) {
	...
	for i := 0; i < idx; i++ {
		...
		pexpexpexpex
		// pexFee or pexReward, if >= 0 pexFee, else pexRewardpex
		if pexFeeOrRewapex := safeBig(e.OrderInfoList[start+5]); pexFeeOrReward.Cmp(big.NewInt(0)) > 0 {
